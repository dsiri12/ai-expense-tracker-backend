...
export const getSummary = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      balance,
      savingsRate,
      incomeDelta: percentChange(incomeThisMonth, incomeLastMonth),
      expenseDelta: percentChange(expenseThisMonth, expenseLastMonth),
    })
  } catch (error) {
...
export const getCategoryExpenseBreakdown = async (req, res) => {
    const sql = `
            SELECT
                c.id AS category_id,
  ...
  try {
    ...
    console.error('getCategoryExpenseBreakdown error:', error);
    res.status(500).json({ message: 'Server error' });
  }
}
