...
const buildMonthlyInsight = async (userId) => {
    const result = await pool.query(
        `WITH current_month_income_expense AS (
            SELECT
        ...
        SELECT
            (SELECT income FROM current_month_income_expense) AS income,
            (SELECT expense FROM current_month_income_expense) AS expense,
            (SELECT json_agg(current_expense_breakdown) FROM current_expense_breakdown) AS breakdown,
            (SELECT json_agg(trend_pre_3_month) FROM trend_pre_3_month) AS trend`,
        [userId]
    );
